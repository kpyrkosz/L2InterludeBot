...
add_oop_dialog::add_oop_dialog(wxWindow* parent, std::function<void(bot_oop_buff&&)> add_handler)
  : wxDialog(parent, wxID_ANY, "Add oop buff", wxDefaultPosition, wxSize(150, 250))
  , add_handler_(add_handler)
{
  ...
  requires_invite = new wxCheckBox(this, wxID_ANY, "Requires party invite?");
  ...
  content_sizer->Add(requires_invite, 0, wxEXPAND);
  ...
  ok_button->Bind(wxEVT_BUTTON, &add_oop_dialog::on_ok, this);
  no_button->Bind(wxEVT_BUTTON, &add_oop_dialog::on_cancel, this);
...
void
add_oop_dialog::on_ok(wxCommandEvent& event)
{
  ...
  add_handler_(bot_oop_buff{true,
                            num_spellid,
                            playername->GetValue().c_str(),
                            buffbefre,
                            is_targetless->GetValue(),
                            requires_invite->GetValue()});
}

void
add_oop_dialog::on_cancel(wxCommandEvent& event)
{
  Close();
}
